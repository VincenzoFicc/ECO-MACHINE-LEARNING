...
# Funzione per prevedere tutti i cambiamenti utilizzando tutte le metriche come feature
def predict(feature_columns, columns, data,):
    future_predictions_dict = {}

    for target in columns:
        print(f"\nPrevedendo per: {target}")

        # Selezionare le feature (tutte le metriche) e il target 
        ...
        print(f"Valori di valutazione per {target}:")
        print(f"R²: {r2}")
        print(f"MSE: {mse}")
        print(f"RMSE: {rmse}")
        print(f"MAE: {mae}")
        ...
        # Prevedere i cambiamenti futuri fino al 2030
        ...
        future_predictions_dict[target] = future_predictions

        # Visualizzare i risultati
        plt.figure(figsize=(10, 6))
        plt.plot(data['Year'], data[target], label='Storico')
    ...
    output_file_path = os.path.join(base_dir, '..', 'Risultati', 'PrevisioniItalia.csv')
